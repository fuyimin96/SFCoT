        ...
        for images, te, filename in tqdm(data_loader):
            ...
            for _ in range(args.epochs):
                ...
                grad = torch.autograd.grad(
                    cost, images_adv_, retain_graph=False, create_graph=False
                )[0]
                if torch.rand(1) < args.TI2p:
                    grad = F.conv2d(
                        grad, stacked_kernel, stride=1, padding="same", groups=3
                    )
...
def lkern(kernlen=15):
    kern1d = 1 - np.abs(
        np.linspace((-kernlen + 1) / 2, (kernlen - 1) / 2, kernlen) / (kernlen + 1) * 2
    )
    kernel_raw = np.outer(kern1d, kern1d)
    kernel = kernel_raw / kernel_raw.sum()
    return kernel
