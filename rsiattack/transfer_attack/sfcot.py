...
from rsiattack.transfer_attack.utils.cam import CAM
from rsiattack.transfer_attack.utils.transfrom import op, op_bk
...
class SFCoT(ATTACK):
    ...
    def SFT(self, images, te, model):
        ...
        for _ in range(args.epochs):
            ...
            if args.no_sp and args.no_fp:
                ...
                loss = F.cross_entropy(output_v3, te.repeat(args.num_copies)).to(
                    args.device
                )
        ...
        if not self.args.no_sp:
            for bat, _ in enumerate(x_copy):
                ...
                chosen = (
                    choice
                    if choice >= 0
                    else np.random.randint(0, high=len(self.op_bk), dtype=np.int32)
                )
                x_copy[bat, :, 0:x1, 0:y1] = self.op_bk[chosen](
                ...
                x_copy[bat, :, x1:x2, 0:y1] = self.op_bk[chosen](
                ...
                x_copy[bat, :, x2:223, 0:y1] = self.op_bk[chosen](
                    x_copy[bat, :, x2:223, 0:y1].unsqueeze(0)
                )
                ...
                x_copy[bat, :, 0:x1, y1:y2] = self.op_bk[chosen](
                    x_copy[bat, :, 0:x1, y1:y2].unsqueeze(0)
                )
                chosen = (
                    choice
                    if choice >= 0
                    else np.random.randint(0, high=len(self.op), dtype=np.int32)
                )
                x_copy[bat, :, x1:x2, y1:y2] = self.op[chosen](
                ...
                x_copy[bat, :, x2:223, y1:y2] = self.op_bk[chosen](
                    x_copy[bat, :, x2:223, y1:y2].unsqueeze(0)
                )
                ...
                x_copy[bat, :, 0:x1, y2:223] = self.op_bk[chosen](
                    x_copy[bat, :, 0:x1, y2:223].unsqueeze(0)
                )
                chosen = (
                    choice
                    if choice >= 0
                ...
                x_copy[bat, :, x1:x2, y2:223] = self.op_bk[chosen](
                    x_copy[bat, :, x1:x2, y2:223].unsqueeze(0)
                )
                chosen = (
                    choice
                    if choice >= 0
                ...
                x_copy[bat, :, x2:223, y2:223] = self.op_bk[chosen](
                    x_copy[bat, :, x2:223, y2:223].unsqueeze(0)
                )
    ...
    def transform(self, x, cam_list):
        ...
        """
        return torch.cat(
            [self.blocktransform(x, cam_list) for _ in range(self.args.num_copies)]
        )
